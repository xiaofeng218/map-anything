...
"""
Co3d Dataset using WAI format data.
"""
...
class Co3dWAI(BaseDataset):
    """
    Co3d dataset containing object centric data
    """
    ...
    def _load_data(self):
        ...
        split_metadata_path = os.path.join(
            ...
            self.split,
            f"co3d_scene_list_{self.split}.npy",
        )
...
def get_parser():
    ...
    parser.add_argument(
        "-rd", "--root_dir", default="/data/dataset/data_WAI/co3d", type=str
    )
    parser.add_argument(
        "-dmd",
        "--dataset_metadata_dir",
        default="/data/dataset/mapanything_meta",
        type=str,
    )
    parser.add_argument(
        "-nv",
...
if __name__ == "__main__":
    ...
    if args.viz:
        rr.script_setup(args, "Co3d_Dataloader")
        rr.set_time_sequence("stable_time", sequence=0)
        rr.log("world", rr.ViewCoordinates.RDF, static=True)

    sampled_indices = np.random.choice(len(dataset), size=4, replace=False)
