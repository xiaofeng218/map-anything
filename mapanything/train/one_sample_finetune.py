...
def train(args):
    ...
    for epoch in range(args.train_params.start_epoch, args.train_params.epochs + 1):
        ...
        train_stats = train_one_epoch(
            ...
            epoch,
            loss_scaler,
            log_writer=log_writer if (epoch+1) % args.train_params.print_freq==0 else None,
            args=args,
...
def print_model_grad_status(model):
    print(f"{'Parameter Name':40s} | {'Requires Grad':12s} | {'Has Grad':8s} | {'Grad Norm'}")
    print("-" * 80)
    ...
    for name, param in model.named_parameters():
        ...
        # 计算梯度范数（grad 为 None 或全 0 时要处理）
        if has_grad:
            grad_norm = param.grad.norm().item()
        else:
            grad_norm = None

        print(f"{name:40s} | {str(requires):12s} | {str(has_grad):8s} | {grad_norm}")
...
def train_one_epoch(
    ...
    for data_iter_step, batch in enumerate(
        ...
        if log_writer is not None:
            ...
            save_views_as_image([result[f"view{i + 1}"]["img"] for i in range(n_views)], "image", os.path.join(args.output_dir, "images/gt_image", f"{epoch_1000x:04d}.png"))
            save_views_as_image([result[f"view{i + 1}"]["input_img"] for i in range(n_views)], "image", os.path.join(args.output_dir, "images/input_image", f"{epoch_1000x:04d}.png"))
            save_views_as_image([result[f"pred{i + 1}"]["img"].permute(0, 3, 1, 2) for i in range(n_views)], "image", os.path.join(args.output_dir, "images/pred_image", f"{epoch_1000x:04d}.png"), normalize_type=None)
            save_views_as_image([result[f"view{i + 1}"]["non_ambiguous_mask"] for i in range(n_views)], "mask", os.path.join(args.output_dir, "images/input_mask", f"{epoch_1000x:04d}.png"))
            save_views_as_image([result[f"pred{i + 1}"]["non_ambiguous_mask"] for i in range(n_views)], "mask", os.path.join(args.output_dir, "images/pred_mask", f"{epoch_1000x:04d}.png"))
